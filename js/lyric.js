...
function scrollLyric(time) {
    ...
    for (var k in rem.lyric) {
        if (k == time) break;
        i++;
        totalh += ($(".lrc-item[data-no='" + i + "']").height() + 15);
    }
    rem.lastLyric = time; // 记录方便下次使用
    $(".lplaying").removeClass("lplaying"); // 移除其余句子的正在播放样式
    $(".lrc-item[data-no='" + i + "']").addClass("lplaying"); // 加上正在播放样式
    var scroll = totalh - ($(".lyric").height() / 2) + 150;
    if (mkPlayer.debug) {
        console.debug("当前播放进度:", rem.audio[0].currentTime, "总进度:", rem.audio[0].duration, "\n当前歌词:", rem.lyric[time], "\n歌词进度：", time, "需要上滑的歌词长度：", totalh);
    }
...
function parseLyric(lrc) {
    ...
    for (var i = 0; i < lyrics.length; i++) {
        ...
        for (var k = 0, h = timeRegExpArr.length; k < h; k++) {
            ...
            var time = min * 60 + sec;
            if (!lrcObj[time]) { lrcObj[time] = clause; } else { lrcObj[time] += "<br>" + clause; }
        }
    }
    return lrcObj;
}
...
//强制修正歌词位置
function syncLyricloca(time) {
    if (rem.lyric === '') return false;

    time = parseInt(time); // 时间取整

    if (rem.lyric === undefined || rem.lyric[time] === undefined) {
        // 当前时间点没有歌词,返回上一节
        time = rem.lastLyric;
    }
    var i = 0; // 获取当前歌词是在第几行
